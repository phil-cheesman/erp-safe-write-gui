"""Pipeline orchestration for reorder quantity uploads."""

from __future__ import annotations

import logging

from estship_uploader import reordqty_validators as validators, reordqty_updater as updater
...
def run_validation(conn, rows: list[tuple], database: str, on_step=None) -> PipelineResult:
    """Run validation steps for reorder quantity upload.
    ...
    try:
        ...
        # Step 5: Before/after reorder qty comparison (informational)
...
def run_upload(conn, expected_rows: int, database: str = "", on_step=None) -> PipelineResult:
    """Run upload steps for reorder quantity. ROLLBACK on error, cleanup in finally."""
    result = PipelineResult()
