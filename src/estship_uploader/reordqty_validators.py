"""Validation steps for reorder quantity (nreordqty) uploads."""
...
def import_to_staging(conn, rows: list[tuple]) -> StepResult:
    ...
    try:
        ...
        cursor.executemany(
            ...
            "(Item_Number, Reorder_Qty) VALUES (?, ?)",
            rows,
        )
        cursor.close()
...
def check_value_changes(conn) -> StepResult:
    """Step 5: Show before/after reorder qty comparison across all in-scope warehouses.

    Results are grouped by (item, current_value) so an item with the same
    reorder qty across all its warehouses produces one line, while an item
    ...
    try:
        ...
        cursor.execute(f"""
            SELECT
                s.Item_Number,
                t.nreordqty AS current_value,
                s.Reorder_Qty AS new_value,
                COUNT(*) AS wh_count
            FROM {STAGING_TABLE} s
            JOIN iciwhs t ON t.citemno = s.Item_Number
            GROUP BY s.Item_Number, t.nreordqty, s.Reorder_Qty
            ORDER BY s.Item_Number, t.nreordqty
        """)
        rows = cursor.fetchall()
        cursor.close()
        ...
        return StepResult(
            "PASS",
            f"Reorder qty changes: {changed_groups} of {total_groups} groups will change",
            details,
        )
    except Exception as e:
...
def check_anomalies(conn) -> StepResult:
    ...
    try:
        ...
        issues = []
        if null_count:
            issues.append(f"{null_count} item(s) will have reorder qty cleared (NULL)")
        if zero_count:
