"""Validation steps for reorder point (nreordpt) uploads."""
...
STAGING_TABLE = "dbo.ReordPtUpload_Staging"
...
def import_to_staging(conn, rows: list[tuple]) -> StepResult:
    ...
    try:
        ...
        cursor.executemany(
            ...
            "(Item_Number, Reorder_Point) VALUES (?, ?)",
            rows,
        )
        cursor.close()
...
def check_value_changes(conn) -> StepResult:
    """Step 5: Show before/after reorder point comparison across all in-scope warehouses.

    Results are grouped by (item, current_value) so an item with the same
    reorder point across all its warehouses produces one line, while an item
    whose warehouses have drifted apart produces one line per distinct current
    value — surfacing the drift naturally.
    """
    try:
        cursor = conn.cursor()
        cursor.execute(f"""
            SELECT
                s.Item_Number,
                t.nreordpt AS current_value,
                s.Reorder_Point AS new_value,
                COUNT(*) AS wh_count
            FROM {STAGING_TABLE} s
            JOIN iciwhs t ON t.citemno = s.Item_Number
            GROUP BY s.Item_Number, t.nreordpt, s.Reorder_Point
            ORDER BY s.Item_Number, t.nreordpt
        """)
        rows = cursor.fetchall()
        cursor.close()
        ...
        return StepResult(
            "PASS",
            f"Reorder point changes: {changed_groups} of {total_groups} groups will change",
            details,
        )
    except Exception as e:
...
def check_anomalies(conn) -> StepResult:
    ...
    try:
        ...
        # Count very large (>1000)
        cursor.execute(f"""
            SELECT COUNT(*) FROM {STAGING_TABLE}
            WHERE Reorder_Point > 1000
        """)
        ...
        issues = []
        if null_count:
            issues.append(f"{null_count} item(s) will have reorder point cleared (NULL)")
        if zero_count:
            issues.append(f"{zero_count} item(s) set to 0")
        if large_count:
            issues.append(f"{large_count} item(s) > 1,000")
