"""Upload steps for reorder point — transaction-wrapped UPDATE logic."""
...
def execute_update(conn) -> tuple[StepResult, int]:
    ...
    Every warehouse row that matches a staged item gets the new reorder point.
    ...
    try:
        ...
        cursor.execute(f"""
            UPDATE t
            SET t.nreordpt = s.Reorder_Point
            FROM iciwhs t
...
def validate_in_transaction(
    ...
    try:
        ...
        cursor.execute(f"""
            ...
            WHERE ISNULL(s.Reorder_Point, -1) != ISNULL(t.nreordpt, -1)
        """)
    ...
    try:
        ...
        cursor.execute(f"""
            SELECT
                ...
                MIN(t.nreordpt) AS min_val,
                MAX(t.nreordpt) AS max_val
            FROM iciwhs t
            WHERE EXISTS (
                SELECT 1 FROM {STAGING_TABLE} s
                WHERE s.Item_Number = t.citemno
        ...
        return StepResult(
            "PASS",
            f"Post-commit verified: {total} rows, reorder points {min_val} to {max_val}",
